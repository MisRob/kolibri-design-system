...
describe.visual('KLogo', () => {
 ...
 describe('Render KLogo with different props', () => {
  ...
  it('Render a logo with background', async () => {
   await renderComponentForVisualTest('KLogo' ,{altText: 'Kolibri Logo with background', size: '150', showBackground: true })
   await takeSnapshot('KLogo - background', snapshotOptions);
  })
  it('Render a logo with rectangular background', async () => {
   await renderComponentForVisualTest('KLogo' ,{altText: 'Kolibri rectangular background', size: '150', backgroundStyle: "rect", showBackground: true })
   await takeSnapshot('KLogo - rectangular background', snapshotOptions);
  })
  it('Render a logo with dimensions', async () => {
   await renderComponentForVisualTest('KLogo' ,{altText: 'Kolibri rectangular background', size: '150', backgroundStyle: "rect", showBackground: true, maxSize: '60px', minSize: '30px'})
 ...
 describe('Render a logo with different color schemes', () => {
  it('color: whiteGrey', async () => {
   await renderComponentForVisualTest('KLogo' ,{altText: 'Kolibri logo - whiteGrey', size: '150', colorScheme: 'whiteGrey', showBackground: true, backgroundStyle: "rect" })
   await takeSnapshot('KLogo - whiteGrey', snapshotOptions);
  })
  it('color: blackGrey', async () => {
   await renderComponentForVisualTest('KLogo' ,{altText: 'Kolibri logo - blackGrey', size: '150', colorScheme: 'blackGrey', showBackground: true, backgroundStyle: "rect" })
