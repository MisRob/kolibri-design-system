...
// ==================================================================
// Document-level listeners and observer state
//
...
//
// The activators observer is shared by all floating elements, and
// is only active when at least one floating element is active or
// non-delegated
...
// these on every batch of DOM mutations- too often to scan the whole
// registry each time.
//
...
// Whether the composable is used anywhere on the page: every caller
// belongs to exactly one entry, so an empty registry means no
// callers
function isRegistryEmpty() {
...
// Observing them allows for simple, native-like, and versatile
...
function syncActivatorsObserver() {
  ...
  // The composable stopped being used anywhere: drop the instance too
...
function onDOMMutations() {
  ...
  // tooltip after its 'v-if' button disappears)
  ...
  // a ':key' change replaces its element) The assumption is that
  // non-delegated elements are few (for many floating elements,
  // the recommendation is to use delegation and in such a case
  // this branch won't run)
  if (!_nonDelegated.size) {
    return;
  }

  // Performance optimization. Rather than querySelector for each
  // activator element, querySelectorAll serves as one shared
  // document scan. Prepared as lazy (as long as the cached
  // activator elements are still connected, it won't need to run).
  // TODO comment on querySelectorAll
  ...
  const sharedQuery = floatingId => {
    if (!activatorEls) {
      ...
      for (let i = 0; i < els.length; i++) {
        ...
        // First match wins, mirroring 'document.querySelector'
        if (id && !activatorEls.has(id)) {
          activatorEls.set(id, els[i]);
        }
      }
    }
    return activatorEls.get(floatingId) || null;
  };
...
function syncActivationListeners(floatingId, query) {
  ...
  const el = entry.currentCaller.delegate ? null : getActivatorEl(floatingId, query);
...
// The optional 'query' replaces the document query used on a cache miss,
// see 'observeActivators' for why
function getActivatorEl(floatingId, query = queryActivatorEl) {
  const entry = _registry[floatingId];
  ...
  const activatorEl = query(floatingId);
  if (entry) {
...
// TODO more the comment to the top of the file
/**
