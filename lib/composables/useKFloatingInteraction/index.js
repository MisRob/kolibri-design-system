...
// several explored, each with its own trade-offs, for making the
// delegation mode affordable - a delegated handler runs for every
// event of its type on the page, so the attribute gives it a very
// cheap check to exit early. Hover tooltips are a very common use
// case and hover events fire often, so this was one of the
// important factors driving the decision. Furthermore, it provides
// the basis for a native-like connection that has no constraints on
// where the activator and floating elements sit in code.
...
// Maintenance watch
//
// Activator elements can appear later, be replaced, or removed.
// Watching for that allows for simple, native-like, and versatile
// public use (a loose connection between activators and their
// floating elements, with no constraints on when they exist) without
// internal leaks and user-facing bugs caused by state that no longer
// matches the DOM. Consumers don't have to check whether an
// activator element exists yet or renew its reference when it is
// replaced.
//
// One shared interval, running only when there is anything to watch
// (a floating element is active, or non-delegated ones exist - the
// exact condition the document-wide MutationObserver here used to be
// gated by). Compared to the observer, the cost is fixed and tiny
// per tick instead of proportional to the page's DOM churn - nothing
// runs during renders, however heavy - at the price of latency: each
// duty below settles within an interval rather than a microtask.

// Cadence of the maintenance watch. Exported for tests.
export const _MAINTENANCE_INTERVAL_MS = 150;
...
function syncMaintenanceWatch() {
  ...
  if (isNeeded && maintenanceWatchId === null) {
    maintenanceWatchId = setInterval(onMaintenanceTick, _MAINTENANCE_INTERVAL_MS);
  }
  if (!isNeeded && maintenanceWatchId !== null) {
...
function onMaintenanceTick() {
  // (1) for active floating elements, deactivate those whose
  // activator element left the DOM (e.g. prevents a stuck open
  // tooltip after its 'v-if' button disappears - no deactivating
  // event ever fires for a removed element). Runs first, so that
  // (2) below can already rebind to a replacement.
  ...
  // activation listeners to whichever activator element now has
  // the floating ID (e.g. prevents an unresponsive activator after
  // a ':key' change replaces its element). The per-tick fast path is
  // a cache-validity check per element; the document query behind it
  // runs only in the gap right after an activator element appeared,
  // moved, or was replaced, see 'getActivatorEl'.
...
// whether anything did. Called from a caller's setup and mount, and on every
// maintenance tick.
//
...
function setCurrentCaller(floatingId, caller, isNewEntry = false) {
  ...
  // A new entry has nothing to reconcile - nothing is attached or active yet,
  // and the caller's own mount attaches the activation listeners. Running the
  // sync here would only waste a document query: this runs during 'setup', when
  // a colocated activator element isn't in the DOM yet.
  if (!isNewEntry) {
...
function queryActivatorEl(floatingId) {
  return document.querySelector(`[${ATTR_FLOATING_ID}="${floatingId}"]`);
}

// Resolution order: the recorded active activator element first (always the
// right one while active, and free), then the cached element while it is
// still connected and bears the floating ID, and a document query only when
// both fail - the gap right after an activator element appeared, moved, or
// was replaced. Steady state never queries.
function getActivatorEl(floatingId) {
  const entry = _registry[floatingId];
  if (!entry) {
    return queryActivatorEl(floatingId);
  }
  if (entry.activeActivatorEl.value) {
    return entry.activeActivatorEl.value;
  }
  const cachedEl = entry.cachedActivatorEl;
  // 'isConnected' so that an activator element replaced in the DOM isn't kept
  // returned; the attribute check also catches the floating ID being changed
  // or removed in place, which the childList MutationObserver this replaced
  // never did
  if (cachedEl && cachedEl.isConnected && cachedEl.dataset.floatingId === floatingId) {
    return cachedEl;
  }

  const activatorEl = queryActivatorEl(floatingId);
...
function resolveActivation(event, interaction) {
  ...
  // floating element that doesn't exist, exit early here.
 ...
 *   be rendered conditionally, and be replaced at any point. In delegated mode
 *   a replacement is picked up on the next interaction; in non-delegated mode
 *   a late or replaced activator element becomes responsive within the
 *   maintenance interval (~150ms) rather than instantly.
 *
 * - While a floating element is active, its activator element leaving the DOM
 *   is watched at the same interval: a floating element whose activator
 *   disappears deactivates within ~150ms.
 *
  ...
  onMounted(() => {
    nextTick(() => {
      if (caller.delegate) {
        ...
        // watch attaches the listeners whenever it appears or gets replaced
        syncActivationListeners(floatingId);
      }
    });
  });
