...
// The maintenance watch reports on an interval, so anything it deactivates or
// attaches has happened once an interval (plus a margin) passes
async function flushMaintenance() {
  await new Promise(resolve => setTimeout(resolve, _MAINTENANCE_INTERVAL_MS + 20));
  await nextTick();
}
