<template>
  <div>
    <h1>Vuetify 1.5 VTooltip (without lazy)</h1>
    <p>
      These tooltips are rendered eagerly, meaning they are in the DOM even when not visible.
    </p>
    ...
    -->
    <div v-for="n in 1000" :key="`v-tooltip-no-lazy-${n}`">
      <VTooltip top>
        <template v-slot:activator="{ on }">
          <span v-on="on">Tooltip {{ n }}</span>
        </template>
        <span>This is a Vuetify tooltip without lazy loading.</span>
      </VTooltip>
    </div>
  </div>
</template>
